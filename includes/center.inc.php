...
elseif(isset($_POST['center_change'])){	
	...
	$query="select center_id from students where stu_id=$id and cid=$course";
	...
	if($count > 0){
		...
		$query="select center_id from students_courses where student_id=$id AND course_id=$course AND sc_status IN(0,4)";
		$res=mysqli_query($conn, $query);
		$count=mysqli_num_rows($res);
		$row = mysqli_fetch_array($res);
		$oldcid=$row['center_id'];
	...
	$query = "select * from center_change where old_center_id=$oldcid AND course_id=$course";
	$res = mysqli_query($conn,$query);
	$check = mysqli_num_rows($res);
	
	if($check > 0){
		$query = "delete from center_change where old_center_id=$oldcid AND course_id=$course";
