					...
					<div class="content-inner">
						<div class="row">
							...
							<div class="col-md-2" style="margin-left:-5px;">
								<button type="submit" class="btn btn-default btn-sm current" id="add" onclick="location.href='#';"><span class="fa fa-plus-circle"><span> Course Addition Requests</button>
							</div>
							<div class="col-md-4">
						...
						<div id="change-wrap">
							...
							<div class="table-wrap"><br>
								<h4>Course Addition Requests</h4>
									...
									<tbody>	
										...
										<?php
											if(isset($_POST['search'])){
												...
												$query = "select * from add_course INNER JOIN courses ON add_course.course_id=courses.course_id INNER JOIN centers on add_course.center_id=centers.center_id INNER JOIN students on student_id=stu_id where add_status=0 AND stu_name LIKE '%$search%' OR stu_roll LIKE '%$search%';";
												
												$result = mysqli_query($conn, $query);
												$count = mysqli_num_rows($result);
											}
											else{
												$query = "select * from add_course INNER JOIN courses ON add_course.course_id=courses.course_id INNER JOIN centers on add_course.center_id=centers.center_id INNER JOIN students on student_id=stu_id where add_status=0;";
												$result= mysqli_query($conn, $query);
											}
															...
															<div class="modal-dialog">
																<div class="modal-content modal-cnt" >
																	...
																	<div class="modal-body">
																		...
																		<form action="includes/stu_add.inc.php" method="POST">
																			<input type="hidden" name="id" value="'.$row['stu_id'].'"/>
																			<input type="hidden" name="course" value="'.$row['course_id'].'"/>
																			<input type="hidden" name="center" value="'.$row['center_id'].'"/>
																			
																			
																			<p>Sure to approve '.$row['stu_name'].'\'s request to add course <b>'.$row['course_name'].'</b> at center <b>'.$row['center_name'].'</b>?</p>
																			<p><b>This cannot be reverted!</b></p>
																			
																			<button class="btn btn-success btn-sm pull-right approve" name="approve_add"><span class="fa fa-thumbs-up"> Approve</button>
																			<div class="clearfix"></div>
																		</form>
																	</div> 																
																</div>
																...
																<div class="modal-content modal-deny">
																	...
																	<div class="modal-body">
																		...
																		<form action="includes/stu_add.inc.php" method="POST">
																			...
																			<p>Sure to deny '.$row['stu_name'].'\'s request to add course <b>'.$row['course_name'].'</b> at center <b>'.$row['center_name'].'</b>?</p>
																			<p><b>This cannot be reverted!</b></p>
																			
																			<button class="btn btn-danger btn-sm pull-right deny" name="deny_add"><span class="fa fa-thumbs-down"> Deny</button>
																			<div class="clearfix"></div>
																		</form>
																	</div> 																
																</div>
	...
	<script>
		window.onload = function () {
			document.getElementById('button').onclick = function () {
				...
				window.location.replace('course_add_requests.php');
			};
		};
	</script>
		...
		echo '			    
		    <div id="success-modal">
				<div class="modalconent">
					<h3 style="color:teal;">Information</h3>
					<hr>	
					<p class="para">Student\'s course addition request was successfully approved.</p> 
					<button id="button" class="btn btn-danger btn-sm pull-right">Close</button>
				</div>
			</div>
		';			
		...
		echo '			    
		    <div id="success-modal">
				<div class="modalconent">
					<h3 style="color:teal;">Information</h3>
					<hr>	
					<p class="para">Student\'s course addition request was successfully denied.</p> 
					<button id="button" class="btn btn-danger btn-sm pull-right">Close</button>
				</div>
			</div>
		';			
