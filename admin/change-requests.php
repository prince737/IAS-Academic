				...
				<div id="content">
					...
					<div class="content-inner">
						<div class="row">
							...
							<div class="col-md-2">
								<button type="submit" class="btn btn-default btn-sm current" id="change" onclick="location.href='#';"><span class="fa fa-exchange"><span> Course Change Requests</button>
							</div>
									...
									<tbody>	
										<?php	
											...
											if(isset($_POST['search'])){
												...
												$query = "select * from course_change INNER JOIN courses ON course_change.old_course_id=courses.course_id INNER JOIN centers on course_change.center_id=centers.center_id INNER JOIN students on student_id=stu_id and change_status=0 where stu_name LIKE '%$search%' OR stu_registrationNo LIKE '%$search%';";
												
												$result = mysqli_query($conn, $query);
												$count = mysqli_num_rows($result);
											}
											else{
												$query = "select * from course_change INNER JOIN courses ON course_change.old_course_id=courses.course_id INNER JOIN centers on course_change.center_id=centers.center_id INNER JOIN students on student_id=stu_id and change_status=0;";
												$result= mysqli_query($conn, $query);
											}
											...
											while($row = mysqli_fetch_array($result)){
												...
												$sql="select course_name from courses where course_id=$cid";
																...
																<div class="modal-content modal-cnt" >
																	...
																	<div class="modal-body">
																		...
																		<form action="includes/stu_update.inc.php" method="POST">
																			...
																			<input type="hidden" name="oldValue" value="'.$row['old_course_id'].'"/>
																			<input type="hidden" name="newValue" value="'.$row['new_course_id'].'"/>
																			
																			
																			<p>Sure to approve '.$row['stu_name'].'\'s request to change his course from <b>'.$row['course_name'].'</b> to <b>'.$row1['course_name'].'</b> at center <b>'.$row['center_name'].'</b>?</p>
																			<p><b>This cannot be reverted!</b></p>
																			
																			<button class="btn btn-success btn-sm pull-right approve" name="approve-course"><span class="fa fa-thumbs-up"> Approve</button>
																			<div class="clearfix"></div>
																		</form>
																	</div> 																
																</div>
																...
																<div class="modal-content modal-deny">
																	...
																	<div class="modal-body">
																		...
																		<form action="includes/stu_update.inc.php" method="POST">
																			...
																			<p>Sure to deny '.$row['stu_name'].'\'s request to change his course from <b>'.$row['course_name'].'</b> to <b>'.$row1['course_name'].'</b> at center <b>'.$row['center_name'].'</b>?</p>
																			<p><b>This cannot be reverted!</b></p>
																			
																			<button class="btn btn-danger btn-sm pull-right deny" name="deny_course"><span class="fa fa-thumbs-down"> Deny</button>
																			<div class="clearfix"></div>
																		</form>
																	</div> 																
																</div>
	...
	<script>
		window.onload = function () {
			document.getElementById('button').onclick = function () {
				...
				window.location.replace('change-requests.php');
			};
		};
	</script>
		...
		echo '			    
		    <div id="success-modal">
				<div class="modalconent">
					<h3 style="color:teal;">Information</h3>
					<hr>	
					<p class="para">Student\'s course change request was successfully approved.</p> 
					<button id="button" class="btn btn-danger btn-sm pull-right">Close</button>
				</div>
			</div>
		';			
